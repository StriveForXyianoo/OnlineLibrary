...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $frontUploadDir = "BookAbstract/"; // Change to your desired directory for front images
    $abstractUploadDir = "BookFront/"; // Change to your desired directory for abstract images
    ...
    if ($section === "Reserve") {
        $table = "main_library_reserve_section";
    } elseif ($section === "Circulation") {
        $table = "main_library_circulation_section";
    } elseif ($section === "Filipinana") {
        $table = "main_library_filipinana_section";
    } elseif ($section === "Reference") {
        $table = "main_library_reference_section";
    } elseif ($section === "Archives") {
        $table = "main_library_archives_section";
    } elseif ($section === "Periodicals") {
        $table = "main_library_periodical_section";
    } elseif ($section === "Thesis Dissertation") {
        $table = "main_library_thesisdissertation_section";
    } elseif ($section === "Fiction") {
        $table = "main_library_fiction_section"; // New section
    }
    ...
    if (move_uploaded_file($_FILES["frontPage"]["tmp_name"], $frontImagePath) && move_uploaded_file($_FILES["abstract"]["tmp_name"], $abstractImagePath)) {
        // Files were successfully uploaded, proceed with database insert
        // Modify your SQL query to insert into the determined table
        $sql = "INSERT INTO $table (bk_title, bk_author, front_image, abstract_image, copyright_date, publisher, place_publication, subject, isbn, call_number, quantity) VALUES ('$bookTitle', '$bookAuthor', '$frontImage', '$abstractImage', '$copyrightDate', '$publisher', '$placeOfPublication', '$subject', '$isbn', '$callNumber', '$bookQuantity')";
...
<div id="MainLibraryForm" class="form-container">
        <!-- Your Records form content goes here -->
        <h2>Add Book in Main Library</h2>
