...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $placeOfPublication = mysqli_real_escape_string($conn, $_POST["placeOfPublication"]);
    ...
    $frontUploadDir = "GraduateSchoolBookFront/"; // Change to your desired directory for front images
    $abstractUploadDir = "GraduateSchoolBookAbstract/"; // Change to your desired directory for abstract images
    ...
    if ($section === "Reserve") {
        $table = "graduateschool_library_reserve_section";
    } elseif ($section === "Circulation") {
        $table = "graduateschool_library_circulation_section";
    } elseif ($section === "Filipinana") {
        $table = "graduateschool_library_filipinana_section";
    } elseif ($section === "Reference") {
        $table = "graduateschool_library_reference_section";
    } elseif ($section === "Archive") {
        $table = "graduateschool_library_archive_section";
    } elseif ($section === "Periodicals") {
        $table = "graduateschool_library_periodical_section";
    } elseif ($section === "Thesis Dissertation") {
        $table = "graduateschool_library_thesisdissertation_section";
    }
    ...
    if (move_uploaded_file($_FILES["frontPage"]["tmp_name"], $frontImagePath) && move_uploaded_file($_FILES["abstract"]["tmp_name"], $abstractImagePath)) {
        // Files were successfully uploaded, proceed with database insert using a prepared statement
        $sql = "INSERT INTO $table (bk_title, bk_author, front_image, abstract_image, copyright_date, publisher, place_publication, subject, isbn, call_number, quantity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = $conn->prepare($sql);

        // Bind parameters and execute the statement
        $stmt->bind_param("ssssssssssi", $bookTitle, $bookAuthor, $frontImage, $abstractImage, $copyrightDate, $publisher, $placeOfPublication, $subject, $isbn, $callNumber, $bookQuantity);
...
<div id="MainLibraryForm" class="form-container">
        <!-- Your Records form content goes here -->
        <h2>Add Book in Graduate School Library</h2>

        <div class="left-form">
       
        <form method="post" action="GradAdd.php" enctype="multipart/form-data">
        <input type="hidden" name="form_type" value="main_library">
<div class="select">
        <label for="section"> Section:</label>
    <select id="section" name="section"  class="custom-select-add">
        <option value="Reserve">Reserve</option>
        <option value="Circulation">Circulation</option>
        <option value="Filipinana">Filipiñana</option>
